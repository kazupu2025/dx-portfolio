"""
B-30 不動産 入居者クレーム管理ダッシュボード（Streamlit）
"""
...
@st.cache_data
def load_tenant_claims_data() -> pd.DataFrame:
    """B-30専用ローダー（キャッシュキー衝突防止）"""
    if not CLEANED_CSV.exists():
        return pd.DataFrame()
    df = pd.read_csv(CLEANED_CSV, encoding="utf-8-sig")
    df["receipt_date"] = pd.to_datetime(df["receipt_date"], errors="coerce")
    for col in ["response_days", "work_hours", "cost_estimate", "is_resolved"]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    return df


@st.cache_data
def load_tenant_claims_property_summary() -> pd.DataFrame:
    """B-30 物件サマリー専用ローダー"""
...
st.title("🏢 B-30 不動産 入居者クレーム管理ダッシュボード")
st.caption("B-30 | 2024年1月 | 物件別クレーム件数・解決率・対応日数・緊急案件アラート")

df = load_tenant_claims_data()
prop_summary = load_tenant_claims_property_summary()
...
with st.sidebar:
    ...
    selected_props = st.multiselect("物件を選択", options=properties, default=properties)

filtered = df[df["property_name"].isin(selected_props)].copy() if selected_props else df.copy()
...
col4.metric("未対応件数", f"{unresolved_count:,} 件",
            delta=f"{unresolved_count} 件要対応" if unresolved_count > 0 else None,
            delta_color="inverse")
...
with tab1:
    ...
    if not prop_summary.empty:
        show_df = (
            prop_summary[prop_summary["property_name"].isin(selected_props)]
            if selected_props else prop_summary
        )
...
if urgent_df.empty:
    ...
    st.dataframe(
        urgent_df[disp_cols].rename(columns={
            "case_no": "案件番号",
            "property_name": "物件名",
            "room_no": "部屋番号",
            "claim_type": "クレーム区分",
    ...
    st.caption(f"緊急・未対応: {len(urgent_df)} 件")
