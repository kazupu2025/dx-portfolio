...
CSV_PATH = BASE / "output" / "instructor_summary_202401.csv"
...
check(2, "summary_csv_exists", CSV_PATH.exists(),
      "instructor_summary_202401.csv が存在しない", "analyze.py を再実行")

if REPORT_PATH.exists():
    text = REPORT_PATH.read_text(encoding="utf-8")

    # 3: "講師" を含む
    check(3, "contains_instructor", "講師" in text,
          "レポートに「講師」が含まれない", "analyze.py のレポート生成を確認")

    # 4: "コマ" または "稼働" を含む
    check(4, "contains_lesson_or_workload", ("コマ" in text or "稼働" in text),
          "レポートに「コマ」または「稼働」が含まれない", "analyze.py のレポート生成を確認")

    # 5: インサイトセクション存在
    check(5, "insight_section_exists",
          "インサイト" in text or "ビジネス" in text or "改善提案" in text,
          "インサイト/改善提案セクションがない", "analyze.py の Section 6 を確認")

    # 6: 数値が含まれる (XXX円 形式)
    has_numeric = bool(re.search(r"\d{1,3}(,\d{3})*", text))
    check(6, "numeric_values_present", has_numeric,
          "レポートに数値が含まれない", "analyze.py のフォーマットを確認")

    # 7: 専門分野セクション存在
    check(7, "specialty_section_exists", "専門分野" in text,
          "専門分野セクションがない", "analyze.py の Section 2 を確認")
...
if CSV_PATH.exists():
    ...
    # 8: CSV行数1以上
    check(8, "summary_csv_row_count", len(df_s) >= 1,
          f"instructor_summary_202401.csv 行数: {len(df_s)}", "analyze.py の集計を確認")
else:
    results.append({
        "id": 8, "name": "summary_csv_row_count", "status": "FAIL",
        "detail": "CSV なし", "fix_hint": "analyze.py を実行",
    })
