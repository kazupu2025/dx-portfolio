# app.py — Streamlit ダッシュボード（B-43 顧客満足度）
# encoding: utf-8
...
st.title("⭐ B-43 サービス 顧客満足度分析ダッシュボード")
st.caption("2024年1月 | CSAT・NPS・担当者別満足度分析")

# ── データロード ────────────────────────────────────────────────────────────
@st.cache_data
def load_customer_satisfaction_data() -> pd.DataFrame:
    """B-43専用ローダー（キャッシュキー衝突防止）"""
    if not CLEANED_FILE.exists():
        return pd.DataFrame()
    return pd.read_csv(CLEANED_FILE, encoding="utf-8-sig")


df_all = load_customer_satisfaction_data()
