...
CSV_PATH = Path("output/cleaned_attendance_202401.csv")
...
if REPORT_PATH.exists():
    ...
    sections = ["従業員別月次残業集計", "残業アラート", "部門別残業サマリー", "日次出勤傾向", "異常値検出", "ビジネスインサイト"]
    ...
    has_hours = bool(re.search(r"\d+\.\d+h", text))
    check(6, "numeric_hours_present", has_hours, "時間数値が含まれない", "format を確認")

    alert_ok = ("アラート" in text and ("危険" in text or "警告" in text or "正常" in text))
    check(7, "alert_section", alert_ok, "残業アラートセクションが不完全", "analyze.py のアラートロジックを確認")
else:
    for i in range(2, 8):
