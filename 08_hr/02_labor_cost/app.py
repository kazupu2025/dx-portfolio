"""
B-24 人事・採用 人件費予実ダッシュボード（Streamlit）
起動: cd 08_hr/02_labor_cost && streamlit run app.py
"""
...
with st.sidebar:
    ...
    selected_depts = st.multiselect("部門", departments, default=departments)
...
col4.metric("超過部門数", f"{over_dept_count} 部門",
            delta=f"{over_dept_count} 件" if over_dept_count > 0 else None,
            delta_color="inverse")
