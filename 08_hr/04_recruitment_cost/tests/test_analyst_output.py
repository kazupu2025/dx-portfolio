...
def test_report_contains_cost(report_text):
    assert "コスト" in report_text, "レポートに「コスト」が含まれていない"
...
def test_channel_csv_has_channel_col(channel_df):
    assert "channel" in channel_df.columns, \
        f"channel 列が存在しない。実際の列: {list(channel_df.columns)}"


def test_report_contains_phase_or_funnel(report_text):
    assert "フェーズ" in report_text or "ファネル" in report_text, \
        "レポートにフェーズ/ファネル分析が含まれていない"
