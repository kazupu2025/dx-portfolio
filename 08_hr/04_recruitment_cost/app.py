"""
B-31 人事採用 採用コスト分析ダッシュボード（Streamlit）
"""
...
@st.cache_data
def load_recruitment_cost_data() -> pd.DataFrame:
    """B-31専用ローダー（キャッシュキー衝突防止）"""
    if not CSV_PATH.exists():
        return pd.DataFrame()
    return pd.read_csv(CSV_PATH, encoding="utf-8-sig")


st.title("👥 B-31 人事採用 採用コスト分析ダッシュボード")
st.caption("B-31 | 2024年1月-3月 | 採用チャネル別コスト・ROI・採用ファネル分析")

df = load_recruitment_cost_data()
...
with tab2:
    ...
    funnel_rows = []
    for phase in PHASES:
        count = int((df_filtered["phase"] == phase).sum())
...
if "cost" in df_filtered.columns:
    ...
    disp_cols = [c for c in [
        ...
        "cost", "phase", "offer_acceptance"
    ] if c in high_cost_df.columns]
    if high_cost_df.empty:
        st.info("採用者がいません。")
    else:
        st.dataframe(
            high_cost_df[disp_cols].rename(columns={
                "apply_date": "応募日",
                "apply_no": "応募番号",
                "channel": "チャネル",
                "job_type": "職種",
